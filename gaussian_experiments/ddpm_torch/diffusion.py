...
from utils.weights_transform_funcs import WeightsTransformFuncsRegistry
...
class GaussianDiffusion:
    ...
    def __init__(self, betas, model_mean_type, model_var_type, loss_type,
        ...
        self.sample_x0_noise_std = float(kwargs.get('sample_x0_noise_std',
                                                    0.0))
        ...
        # self.tm = TwoMoons()
        assert kwargs.get('weights_transform_fn') is not None
        self.weights_transform_fn_name = kwargs.get('weights_transform_fn')
        weights_transform_fns = []
        for fn_name in self.weights_transform_fn_name.split('-'):
            weights_transform_fns.append(
                WeightsTransformFuncsRegistry.get(fn_name))
        # Compose the requested transforms in the given order, e.g.
        ...
        self.weights_transform_fn = _compose_transforms(weights_transform_fns)

        self.energy_fn_name = kwargs.get('energy_fn')
        if self.energy_fn_name is not None:
            self.energy_fn = EnergyFunctionsRegistry.get(self.energy_fn_name)
        else:
            self.energy_fn = None
    ...
    def reverse_sample(self, x_t, t, noise=None):
        ...
        coef1 = self._extract(self.sqrt_recip_alphas_bar, t,
                              x_t)  # 1 / \sqrt{\bar \alpha_t}
        coef2 = self._extract(self.sqrt_recip_m1_alphas_bar, t,
                              x_t)  # \sqrt{1 / \bar\alpha_t - 1}
    ...
    def train_losses(self, denoise_fn, x_0, t, noise=None, epoch=None):
        if noise is None:
            noise = torch.randn_like(x_0)
        ...
        if self.loss_type == "kl":
        ...
        elif self.loss_type == "reweighting_data":
            assert self.energy_fn is not None
            weights = self.energy_fn(x_0)
            if weights.ndim == 1:
                weights = weights.unsqueeze(
                    1)  # (B,) -> (B, 1), broadcasting to all dims
            if self.model_mean_type == "mean":
                target = self.q_posterior_mean_var(x_0=x_0, x_t=x_t, t=t)[0]
            elif self.model_mean_type == "x_0":
                target = x_0
            ...
            if epoch is not None and epoch < self.reweighting_rl_warmup_epochs:
                # first 20 epochs, use dataset data to initialize the policy
                sample_x0 = x_0
                sample_xt = x_t
                weights = torch.ones_like(x_0)
            else:
                sample_x0 = self.p_sample(denoise_fn,
                                          shape=x_0.shape,
                                          device=x_0.device,
                                          noise=None)
                sample_x0 = sample_x0.clamp(-6., 6.)
                if self.sample_x0_noise_std > 0:
            ...
            if weights.ndim == 1:
                weights = weights.unsqueeze(
                    1)  # (B,) -> (B, 1), broadcasting to all dims
            if self.model_mean_type == "mean":
                target = self.q_posterior_mean_var(x_0=sample_x0,
                                                   x_t=sample_xt,
                                                   t=t)[0]
            elif self.model_mean_type == "x_0":
                target = sample_x0
