...
for initial_mean in 0.0; do
    EXP_STR="shape_cusps_v2_exploration_reproduce"
    GROUP_NAME="shape_cusps_v2_exploration_reproduce"
    for temperature in 0.1; do
        # for energy_fn_kwargs_l in 1.0 1.2 1.4; do
        for weights_transform_kwargs_min_value in -0.4; do
            for energy_fn_kwargs_l in 0.6; do
                for seed in 45 46; do
                    # for l in 0.1 0.2 0.3; do
                    for weights_transform_fn in "group_relative_negative_quadratic" "softmax_negative"; do
